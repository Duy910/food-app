...
function MenuMain({ data }) {
    ...
    const products = foodItems?.filter(foodItem => foodItem.category === acticve)
            ...
            <div className='bg-menudetail '>
                            ...
                            {products && products.map((product) => {
                                ...
                                className=''>
                                    <a
                                        ...
                                        className={` font-bold font-delius text-[16px] rounded-[40px]  px-6 py-4 uppercase ${active === product.urlParamName ? 'activeMenu' : ''}`}>{category.urlParamName}</a>
                                </li>
                            })}
                ...
                <div className='max-w-[1200px] my-0 mx-auto' >
                    <div className=' flex flex-wrap  mb-20'>
                        {pizza && pizza.map((foodItem) => {
                            return (
                                <div key={foodItem.id} className='py-6 hover:shadow-2xl hover:rounded-[10px] hover:bg-[#fff8ef] w-[600px] ' >
                                    <div className='flex px-2'>
                                        <div className=' mr-3 '>
                                            <img className='bg-contain overflow-hidden max-w-[120px] max-h-[120px]' src={foodItem.imgURL} />
                                        </div>
                                        <div className='relative overflow-hidden menuCart hover: cursor-pointer'>
                                            <div className='flex text-[25px] font-delius font-semibold'>
                                                ...
                                                <h1 className='ml-4 text-primary'> <FontAwesomeIcon icon={faEuroSign} />{foodItem.price}</h1>

                                            </div>
                                            <h1 className='italic text-[#666] font-rubik font-semiblod'>{foodItem.des}
                                            </h1>
                                            <div className='' >
